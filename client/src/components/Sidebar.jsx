...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  if (!confirmDelete) return Promise.reject(new Error('Deletion cancelled'));

  return axios
    .post('/api/chat/delete', { chatId }, { headers: { Authorization: `Bearer ${token}` } })
    .then(async ({ data }) => {
      if (data.success) {
        setChats(prev => prev.filter(chat => chat._id !== chatId));
        await fetchUsersChats();
        return data.message; // success message for toast.promise
      } else {
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen &&
      ...
      {/*Logo*/}
      <img src={theme === 'dark' ? assets.logo_full_dark : assets.logo_full} alt=''
            ...
            flex justify-between group'>
              ...
              w-4 cursor-pointer not-dark:invert' alt=""
                    onClick={e => toast.promise(deleteChat(e, chat._id), {
                      loading:
                        'deleting...',
                        success:(msg) => msg || 'Chat deleted Successfully!',
      ...
      dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-gray-400'>Purchase credits to use AIRA pro</p>
        </div>
      </div>
      ...
      dark:border-white/15 rounded-md'>
        ...
        <label className='relative inline-flex cursor-pointer'>
          <input onChange={() => setTheme(theme === 'dark' ? 'light' : 'dark')} type="checkbox" className='sr-only peer' checked={theme ===
            'dark'} />
